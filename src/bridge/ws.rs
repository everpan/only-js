//! ws.* 绑定：WebSocket 帧循环内 JS 主动控制（移植 Go bridge/ws.go 的 jsWS）。
//!
//! 仅两个 op（对齐 Go）：send(data) 收集到 ReqState.ws_sends（帧处理器结束后按序写出）、
//! close() 置位 ReqState.ws_close（本帧结束后关连接）。HTTP 请求路径不读这两项（等价 Go nil 连接 no-op）。
