...
mod tests {
    ...
    async fn real_postgres_roundtrip_via_vtable() {
        ...
        drive(&mut exec(handle, RString::from("insert into oj_plugin_t (id, v) values ($1, $2)"), RString::from(r#"[1,"hi"]"#)))
            .await
            .expect("insert");

        let rows = drive(&mut query(handle, RString::from("select v from oj_plugin_t where id = $1"), RString::from(r#"[1]"#)))
            .await
            .expect("query");
        ...
        drive(&mut tx_exec(handle, tx_id, RString::from("insert into oj_plugin_t (id, v) values ($1, $2)"), RString::from(r#"[2,"tx"]"#)))
            .await
            .expect("tx insert");
        ...
        let tx_id = serde_json::from_slice::<serde_json::Value>(&bytes).unwrap()["tx_id"].as_u64().unwrap();
        drive(&mut tx_exec(handle, tx_id, RString::from("insert into oj_plugin_t (id, v) values ($1, $2)"), RString::from(r#"[3,"rb"]"#)))
            .await
            .expect("tx insert2");
        ...
        close(handle);
        drive(&mut query(handle, RString::from("select 1"), RString::from("[]")))
            .await
