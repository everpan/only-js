...
mod tests {
    ...
    async fn ws_echo_roundtrip_on_pinned_thread() {
        ...
        let addr = spawn(
            app("/v1/api", t.0.clone(), true, crate::tests::build_table(&t.0, true, "/v1/api"), crate::tests::actor(t.0.clone(), true), None, None).merge(echo_route()),
        )
        .await;
    ...
    async fn js_route_runs_handler_per_frame() {
        ...
        let addr = spawn(
            app("/v1/api", t.0.clone(), true, crate::tests::build_table(&t.0, true, "/v1/api"), crate::tests::actor(t.0.clone(), true), None, None).merge(js_route(
                "/ws/js",
                handler.clone(),
